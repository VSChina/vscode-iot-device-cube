...
export class FSPromise {
  static async isDirectory(filePath: string): Promise<boolean> {
    return new Promise((resolve: (isDirectory: boolean) => void, reject: (reason: Error) => void) => {
  ...
  static async listTree(filePath: string): Promise<string[]> {
    return new Promise((resolve: (files: string[]) => void, reject: (reason: Error) => void) => {
